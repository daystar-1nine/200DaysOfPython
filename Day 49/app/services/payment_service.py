...
# Objective  : Provide credit card processing interface intended for external API mocking tests with logging.
# Concept    : External Service Integration & Mocking Target
# Why Used   : Demonstrates mocking third-party payment services without logging sensitive card numbers.
# ==============================================================================
...
class PaymentGatewayClient:
    ...
    def charge(self, amount: float, currency: str, card_number: str) -> dict:
        ...
        if card_number == "0000000000000000":
            logger.warning("Payment processing declined by gateway for test card")
...
class PaymentService:
    ...
    def process_payment(self, req: PaymentChargeRequest) -> PaymentResponse:
        # Mask credit card number in logs (only display last 4 digits)
        masked_card = f"****-****-****-{req.card_number[-4:]}"
        logger.info(f"Processing payment amount={req.amount} {req.currency} for card={masked_card}")
        ...
        logger.info(f"Payment successful: tx_id='{res_data['transaction_id']}'")
