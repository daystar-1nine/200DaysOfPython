...
def test_create_order_insufficient_stock_returns_409(client, sample_product, normal_user_headers):
    """Test ordering more units than available stock returns HTTP 409 Conflict with INSUFFICIENT_STOCK code."""
    payload = {
        "items": [
    ...
    assert "insufficient stock" in data["error"]["message"].lower()
