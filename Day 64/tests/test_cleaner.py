...
def test_clean_sales_data_segment_derivation(sample_sales_df):
    cleaned = clean_sales_data(sample_sales_df)
...
def test_clean_sales_data_deduplication(sample_sales_df):
    duplicated = pd.concat([sample_sales_df, sample_sales_df.head(5)], ignore_index=True)
    cleaned = clean_sales_data(duplicated)
