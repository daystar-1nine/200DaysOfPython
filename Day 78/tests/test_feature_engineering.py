...
def test_engineer_features_adds_columns(sample_df):
    ...
    expected = ["Charges_Per_Month_Ratio", "Support_Per_Tenure", "Complaint_Risk_Index", "Net_Monthly_Charges", "Tenure_Age_Ratio"]
    for col in expected:
...
def test_engineer_net_monthly_charges(sample_df):
    df_feat = engineer_features(sample_df)
    expected = sample_df["Monthly_Charges"] * (1.0 - sample_df["Discount"] / 100.0)
    assert (df_feat["Net_Monthly_Charges"].round(4) == expected.round(4)).all()
