...
# Objective  : ProductService business logic layer.
# Concept    : Product Business Logic
# Why Used   : Enforces product creation, update, and deletion validation rules.
# ==============================================================================
...
class ProductService:
    ...
    def create_product(self, payload: ProductCreate) -> Product:
        return self.repository.create(name=payload.name, price=payload.price, stock=payload.stock)

    def replace_product(self, product_id: int, payload: ProductUpdate) -> Product:
        product = self.repository.update(product_id, payload.name, payload.price, payload.stock)
        if not product:
            raise ProductNotFoundError(product_id)
        return product
    ...
    def patch_product(self, product_id: int, payload: ProductPatch) -> Product:
        ...
        return self.repository.update(product_id, name, price, stock)
