...
def test_load_data_valid(sample_csv_file):
    """
    Test loading a valid sales CSV file.
    """
    ...
    assert "Revenue" not in df.columns  # Raw CSV should not have Revenue yet
...
def test_load_data_missing_file_raises_error(tmp_path):
    ...
    missing_path = tmp_path / "non_existent_sales.csv"
...
def test_load_data_missing_columns_raises_error(tmp_path):
    ...
    # How it works: Writes CSV missing 'Quantity' and 'Unit_Price' columns.
    invalid_csv = tmp_path / "invalid_schema.csv"
    invalid_df = pd.DataFrame({"Order_ID": [1001], "Customer_Name": ["Test"]})
