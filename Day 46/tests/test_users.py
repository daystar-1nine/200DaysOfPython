...
# Objective  : Integration testing of User CRUD endpoints with phone numbers and creation timestamps.
# Concept    : API Integration Testing with Migrated Fields
# Why Used   : Asserts user creation, duplicate email rejection, and phone number persistence.
# ==============================================================================

def test_api_read_root(client):
    res = client.get("/")
    assert res.status_code == 200
    assert "Mini E-Commerce Backend V2" in res.json()["message"]

def test_create_user(client):
    res = client.post("/users", json={"name": "Suraj Sawant", "email": "suraj@example.com", "phone": "+91-9876543210"})
...
def test_get_user(client):
    res_c = client.post("/users", json={"name": "Alex Mercer", "email": "alex@example.com", "phone": "1234567890"})
